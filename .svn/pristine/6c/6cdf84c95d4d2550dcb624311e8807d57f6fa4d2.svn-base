...
<script>
    export default{
        ...
        beforeRouteLeave(to, from, next) {
            // 设置下一个路由的 meta
           from.query.categoryName = this.name;
           from.query.categoryId = this.id;
           next();
          },
        methods: {
            select(i,e,event){
            ...
            getData(value){
                ...
                this.$http.get("/API/baseservice/GetCatogary", {params:{catogaryName:value}},{ emulateJSON: true })
                .then(res => {
