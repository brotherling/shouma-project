  ...
  <v-header :header="parentMsg"></v-header>
    <section class="assetcontent" @scroll="handleScroll($event)">
        ...
        <div class="statistics">
          <div><span>总数量:</span>{{quantity}}</div>
          <div><span>总原值:</span>{{assetOriWorth}}</div>
        </div>
        <div class="assetpagedata" id="assetpage">
            <div class="pagedata" v-for="item in assetedata" :id="'a'+item.ASM_A1_10" :key="item.ASM_A1_10">
                <ul  @click="showdata(item.ASM_A1_10)">
                    <li v-for="(tou) in headtdata" :key="tou.field">
                      <span class="Amendfont">{{tou.title}}:</span><span style="margin-left:15px;">{{renderData(tou.field,item)}}</span>
                    </li>
    ...
    <div id="zhezhao"></div>
  <v-foot @refreshbizlines="clicksao"></v-foot>
</div>
</template>
<script>
export default {
    ...
    UploadData() {
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.$http
        .post("/API/assetservice/GetAssetTitle", { emulateJSON: true })
        .then(res => {
          let data = JSON.parse(res.data);
                this.Imgshow=true;
          this.scloll = true;
